...
const BootupTime = ({ data }) => {
	...
	return (
		<div className="p-4 bg-white rounded-lg shadow-md">
			...
			<p className="mb-4">JavaScript Execution Time: {displayValue}</p>
					...
					<tbody>
						{details?.items?.map((item, index) => (
							<tr key={index} className="hover:bg-gray-50">
								...
								<td className="py-2 px-4 border-b border-gray-300">
									{item?.total?.toFixed(2)} ms
								</td>
								<td className="py-2 px-4 border-b border-gray-300">
									{item?.scripting?.toFixed(2)} ms
								</td>
								<td className="py-2 px-4 border-b border-gray-300">
									{item?.scriptParseCompile?.toFixed(2)} ms
								</td>
							</tr>
						))}
					</tbody>
