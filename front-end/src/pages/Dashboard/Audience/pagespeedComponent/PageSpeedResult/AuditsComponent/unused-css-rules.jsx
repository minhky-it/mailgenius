...
const UnusedCSSRules = ({ data }) => {
	const { title, description, score, displayValue, details } =
		data;
	...
	return (
		<div className="p-4 bg-white rounded-lg shadow-md">
			...
			<h3 className="text-lg font-semibold mt-4">Unused CSS Details</h3>
					...
					<tbody>
						{details?.items?.map((item, index) => (
							<tr key={index} className="hover:bg-gray-50">
								...
								<td className="py-2 px-4 border-b border-gray-300">
									{(item?.wastedBytes / 1024)?.toFixed(2)} KB
								</td>
							</tr>
						))}
					</tbody>
