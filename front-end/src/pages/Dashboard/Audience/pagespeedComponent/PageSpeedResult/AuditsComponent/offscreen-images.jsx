...
const OffscreenImages = ({ data }) => {
	...
	return (
		<div className="p-4 bg-white rounded-lg shadow-md">
			...
			<p className="mb-2">FCP Savings: {metricSavings?.FCP} ms</p>

			<h3 className="text-lg font-semibold mt-4">Offscreen Images</h3>
					...
					<tbody>
						{details?.items?.map((item, index) => (
							<tr key={index} className="hover:bg-gray-50">
								...
								<td className="py-2 px-4 border-b border-gray-300">
									{item?.wastedBytes?.toLocaleString()} bytes
								</td>
							</tr>
						))}
					</tbody>
